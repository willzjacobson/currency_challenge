...
exports.calculateCost = (date, daysRemaining, totalCost = 0) => {
  ...
  const lastPurchaseDayOfMonth =
    daysRemaining >= daysLeftInMonth
      ? daysInMonth
      : dayAsNumber + daysRemaining - 1;

  totalCost += exports.calculateCostFromThisMonth(
    month,
    year,
    dayAsNumber,
    lastPurchaseDayOfMonth
  );
  ...
  return exports.calculateCost(
    `${updatedMonth}/01/${updatedYear}`,
    daysRemaining,
    totalCost,
    false
  );
...
exports.tickMonth = (currentYear, currentMonth) => {
  const m = moment(`${currentYear}-${currentMonth}`, 'YYYY-MM').add(
    1,
    'months'
  );
...
exports.calculateCostFromThisMonth = (
  month,
  year,
  startDayThisMonth,
  endDayThisMonth
  ...
  for (let i = startDayThisMonth; i <= endDayThisMonth; i++) {
    ...
    const dayOfWeek = moment(
      `${year}-${month}-${exports.formatDayOrMonth(i)}`,
      'YYYY-MM-DD'
    ).day();
